...
/* Linear-rescale forward (Rabiner scaling). Instead of storing the log-space
   forward Alphamat[t][i] = log(alpha) + Sigma_{t-1} (which a float matrix
   cannot hold: |Sigma| ~ 1e6 on a real chromosome swamps the O(10) signal in
   float's low bits -> ~6% error), store the NORMALIZED forward ahat[t][i] =
   alpha[t][i]/S_t (O(1)-ranged, float-friendly to 1e-7 relative) and keep the
   big magnitude in a DOUBLE per-locus logScale[t]. The relation is
   Alphamat[t][i] = log(ahat[t][i]) + logScale[t], so the backward recovers any
   exp(Alphamat[t][i]+Z) as ahat[t][i]*exp(logScale[t]+Z). The recursion runs in
   double (a 2-column rolling buffer); only the stored matrix is float. Bonus:
   no per-element exp()/log() (the ~24% libm hotspot) -- the read is a multiply
   and the store a divide; one log() per locus. Validated bit-for-bit (at
   calibration precision) against the log-space scheme in NumPy; see
   docs/float-alphamat-linear-rescale.md. */
  ...
  for(i=0; i < *p_Nhaps; i++)
    {
      ...
      // exp(Alphamat[t][i]+Z) == ahat[t][i]*exp(logScale[t]+Z); ahat is the
      // stored normalized float, logScale[t] the cumulative double normalizer.
...
double ** sampler(double ** copy_prob_new_mat, signed char * newh, signed char ** existing_h, int *p_Nloci, int *p_Nhaps, int *p_nchr, double p_rhobar, double * MutProb_vec, int * allelic_type_count_vec, double * lambda, double * pos, double * copy_prob, double * copy_probSTART, int * pop_vec, int * cond_mat_haplotypes,int ndonorpops, int run_num, int ind_val, struct files_t *Outfiles, struct param_t *Par)
{
  ...
  double large_num;
  // ahat is COLUMN-MAJOR (haps): ahat[locus] is a contiguous array of
  // n_haps FLOATS (the normalized forward; linear-space rescale). The hot
  // inner loop iterates `i` for fixed `locus`, so consecutive `i` accesses
  // land on adjacent cache-line bytes -> vectorizable, prefetcher-friendly.
  // One huge contiguous storage block (~12 GB for chr1, half the old double
  // matrix) avoids the jagged-2D TLB pressure of the upstream code.
  // logScale[t] is the per-locus cumulative log-normalizer (double, tiny:
  // Nloci doubles) carrying the large magnitude float can't hold.
  ...
  // Each ahat[locus] points to the locus-th column of n_haps floats
  // within Alphamat_storage. We reuse `i` as a generic counter for
  // `locus` here to avoid declaring a new variable.
  for(i=0 ; i< *p_Nloci ; i++)
    {
  ...
  //////////////////////////////// 
   ...
   if (finalrun)
       ...
       for (i=0; i < *p_Nhaps; i++)
	 {
       ...
       /* -s>0 path sampling (rarely used; calibration runs -s 0). It reads the
          forward matrix by random access. ahat stores the normalized float; the
          old double Alphamat[L][I] == log(ahat[L][I]) + logScale[L], and every
          use below is a within-column max-shifted ratio where logScale[L]
          cancels exactly, so AMAT reconstructs it faithfully (one log per access
          on this cold path only). */
#define AMAT(L,I) (log((double)ahat[(L)][(I)]) + logScale[(L)])
       if (Par->samplesTOT > 0) {
       ...
       for (locus=0; locus < *p_Nloci; locus++)
	 {
	   ...
	   for (i = 1; i < *p_Nhaps; i++)
	     {
	       if (AMAT(locus,i) > large_num)
		 large_num = AMAT(locus,i);
	     }
	   large_num = -1.0*large_num;
	   for (i = 0; i < *p_Nhaps; i++)
	     Alphasumvec[locus] = Alphasumvec[locus] + exp(AMAT(locus,i)+large_num);
       ...
       for (j = 0; j < Par->samplesTOT; j++)
	 {
	   ...
	   for (i = 1; i < *p_Nhaps; i++)
	     {
	       if (AMAT((*p_Nloci-1),i) > large_num)
		 large_num = AMAT((*p_Nloci-1),i);
	     }
	   large_num = -1.0*large_num;
	   ...
	   for (locus = (*p_Nloci-2); locus >= 0; locus--)
	     {
	       ...
	       total_prob = log(exp(Alphasumvec[locus]+large_num)*TransProb[locus]*copy_prob[sample_state[(locus+1)]] + exp(AMAT(locus,sample_state[(locus+1)])+large_num)*(1.0-TransProb[locus]))-large_num;
	       no_switch_prob = exp(log(exp(AMAT(locus,sample_state[(locus+1)])+large_num)*(1.0-TransProb[locus])) - large_num - total_prob);
